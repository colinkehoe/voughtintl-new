  ...
  run: async ({ client, interaction }) => {
    ...
    if (
      ...
      const embed = new EmbedBuilder() //create embed
        ...
        .setDescription(
          `Should we welcome ${interaction.user.tag} to the Church of the Collective?`
        )
      ...
      const filter = (reaction) => {
        //create reaction filter: only allow 👍 and 👎
        return reaction.emoji.name === "👍" || reaction.emoji.name === "👎";
      };
      let collector = poll.createReactionCollector({
        filter, //create reaction collector for 24 hours
        time: 10000,
      });
      ...
      collector.on("collect", (r) => {
        //log reactions
        console.log(
          `Collected ${r.emoji.name} from ${r.users.cache.last().tag}`
        );
      });
      collector.on("end", (collected) => {
        //log collected reactions
        ...
        if (votesyes > votesno) {
          ...
          interaction.channel.send(
            `Congratulations ${interaction.user.tag}, you have been deemed worthy of the Fresca role.`
          );
        } else {
          user.roles.add("No Fresca"); //give no fresca role if user does not have majority of votes
          interaction.channel.send(
            `Sorry ${interaction.user.tag}, you have been deemed unworthy of the Fresca role. Have the No Fresca role as punishment.`
          );
        }
      });
    } else {
