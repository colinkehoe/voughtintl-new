...
const mute: SlashCommand = {
    ...
    run: async (client: Client, interaction: ChatInputCommandInteraction) => {
        ...
        if (target.bot)
            return interaction.editReply("You can't mute me!");
        
        const member: GuildMember | undefined = await interaction.guild?.members.fetch(target.id);
        if (!member)
        ...
        if (!mutedRole)
            return interaction.editReply("There is no muted role in this server!");
