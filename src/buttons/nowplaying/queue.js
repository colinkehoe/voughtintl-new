...
module.exports = {
    run: async ({ client, button }) => {
        ...
        if (!queue)
            return button.reply.send('There is no music being played right now!');
        ...
        var embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Queue requested by ${button.user.tag}`, iconURL: button.user.avatarURL() })
            .setTimestamp();
        ...
        const collector = button.channel.createMessageComponentCollector({ filter: i => i.customId == 'first' || i.customId == 'next' || i.customId == 'previous' || i.customId == 'last', time: 60000 });
