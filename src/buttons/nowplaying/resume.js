        ...
        return button.update({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(`Currently playing: ${queue.current.author} -- [${queue.current.title}](${queue.current.url})`)
                    .setFooter({ text: `Requested by ${interaction.user.username}`, iconURL: queue.current.requestedBy.avatarURL() })
                    .setTimestamp()
            ],
            components: [row]})
    }
